                    ...
                    CreateScreen( activityViewModel, onEvent = { event ->
                        when (event) {
                            ...
                            is CreateEvent.CreateActivity ->{
                                ...
                                activityViewModel.addActivity(Activity(id=id, creator_id = if (authViewModel.currentUser==null){""}else{
                                    authViewModel.currentUser!!.uid.toString()}, title =event.title ,date=event.date, start_time =event.start_time, time_length = event.time_length))
